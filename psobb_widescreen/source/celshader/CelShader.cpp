...
CelShader::CelShader(IDirect3DDevice9 *device, int width, int height)
        : device(device),
    ...
    defines.push_back(pixelSizeMacro);
		...
		/**
		 * IMPORTANT! Here we load and compile the CelShader effect from a *RESOURCE*
		...
		V(D3DXCreateEffectFromResourceA(device, g_hModule, "CelShader.fx", &defines.front(), &includeResource, flags, nullptr, &effect, nullptr));
		//V(D3DXCreateEffectFromFileW(device, L"CelShader.fx", &defines.front(), nullptr, flags, nullptr, &effect, nullptr));
...
void CelShader::go(IDirect3DTexture9 *frame, IDirect3DSurface9 *dst) {
    HRESULT hr;
...
void CelShader::toneMapPass(IDirect3DTexture9* frame, IDirect3DSurface9 *dst) {
    D3DPERF_BeginEvent(D3DCOLOR_XRGB(0, 0, 0), L"CelShader: 1st pass");
    HRESULT hr;
...
void CelShader::quad(int width, int height) {
