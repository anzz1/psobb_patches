...
Direct3DDevice8::Direct3DDevice8(Direct3D8 *d3d, IDirect3DDevice9 *ProxyInterface, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS *PresentParams) :
	D3D(d3d), ProxyInterface(ProxyInterface)
{
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetDeviceCaps(D3DCAPS8 *pCaps)
{
	...
	const HRESULT hr = ProxyInterface->GetDeviceCaps(&devCaps);
	if (FAILED(hr))
		return hr;

	ConvertCaps(devCaps, *pCaps);
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS8 *pPresentationParameters, IDirect3DSwapChain8 **ppSwapChain)
{
	...
	*ppSwapChain = nullptr;
	...
	DWORD MSQuality = 0;
	if (g_bMSAA) {
		//if (SUCCEEDED(D3D->GetProxyInterface()->CheckDeviceMultiSampleType(CreationParams.AdapterOrdinal, CreationParams.DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_8_SAMPLES, &MSQuality)))
			//PresentParams.MultiSampleType = D3DMULTISAMPLE_8_SAMPLES;
		if (SUCCEEDED(D3D->GetProxyInterface()->CheckDeviceMultiSampleType(CreationParams.AdapterOrdinal, CreationParams.DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_4_SAMPLES, &MSQuality)))
			PresentParams.MultiSampleType = D3DMULTISAMPLE_4_SAMPLES;
		else if (SUCCEEDED(D3D->GetProxyInterface()->CheckDeviceMultiSampleType(CreationParams.AdapterOrdinal, CreationParams.DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_2_SAMPLES, &MSQuality)))
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetRenderTarget(IDirect3DSurface8 *pRenderTarget, IDirect3DSurface8 *pNewZStencil)
{
	...
	if (pRenderTarget != nullptr)
	{
		...
		// rt 1 = unit shadows
		// rt 2 = world
		// rt 3 = ??? (1 texture, 1 trianglefan, 20 calls)
		// rt 4 = hud
		//
		// (mrts == 2)
		// rt 1 = world
		...
		if (nrts >= 2 && nrts == mrts) {
			...
			ProxyInterface->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);
			ProxyInterface->SetRenderState(D3DRS_COLORWRITEENABLE, D3DCOLORWRITEENABLE_RED | D3DCOLORWRITEENABLE_GREEN | D3DCOLORWRITEENABLE_BLUE);
	...
	if (pNewZStencil != nullptr)
	{
		...
		pMainDepthStencilSurf = pNewZStencilImpl->GetProxyInterface();
	}
	else
	{
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value)
{
	...
	case D3DTSS_MAGFILTER:
	{
		ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAXANISOTROPY, MIN(ANISOTROPY_LEVEL, devCaps.MaxAnisotropy));
		HRESULT hr = ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC); // anisotropic filtering
		return hr;
	}
	case D3DTSS_MINFILTER:
	{
		ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAXANISOTROPY, MIN(ANISOTROPY_LEVEL, devCaps.MaxAnisotropy));
		HRESULT hr = ProxyInterface->SetSamplerState(Stage, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC); // anisotropic filtering
		return hr;
	}
	case D3DTSS_MIPFILTER:
		return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR); // trilinear filtering
	case D3DTSS_MIPMAPLODBIAS:
	...
	case D3DTSS_MAXANISOTROPY:
		return ProxyInterface->SetSamplerState(Stage, D3DSAMP_MAXANISOTROPY, MIN(ANISOTROPY_LEVEL, devCaps.MaxAnisotropy));
	default:
...
void Direct3DDevice8::StoreRenderState() {
	prevStateBlock->Capture();
	prevVDecl = NULL;
	prevDepthStencilSurf = NULL;
	GetProxyInterface()->GetVertexDeclaration(&prevVDecl);
	GetProxyInterface()->GetDepthStencilSurface(&prevDepthStencilSurf);
	GetProxyInterface()->SetDepthStencilSurface(depthStencilSurf);
}

void Direct3DDevice8::RestoreRenderState() {
	if(prevVDecl) {
		GetProxyInterface()->SetVertexDeclaration(prevVDecl);
		prevVDecl->Release();
	}
	GetProxyInterface()->SetDepthStencilSurface(prevDepthStencilSurf); // also restore NULL!
...
IDirect3DTexture9* Direct3DDevice8::GetSurfaceTexture(IDirect3DSurface9* pSurface) {
	IUnknown *pContainer = NULL;
	HRESULT hr = pSurface->GetContainer(IID_IDirect3DTexture9, (void**)&pContainer);
	if(hr == D3D_OK) return (IDirect3DTexture9*)pContainer;
