...
HRESULT STDMETHODCALLTYPE Direct3D8::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS8 *pPresentationParameters, IDirect3DDevice8 **ppReturnedDeviceInterface)
{
	...
	*ppReturnedDeviceInterface = nullptr;
	...
	DWORD MSQuality = 0;
	if (g_bMSAA) {
		//if (SUCCEEDED(ProxyInterface->CheckDeviceMultiSampleType(Adapter, DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_8_SAMPLES, &MSQuality)))
			//PresentParams.MultiSampleType = D3DMULTISAMPLE_8_SAMPLES;
		if (SUCCEEDED(ProxyInterface->CheckDeviceMultiSampleType(Adapter, DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_4_SAMPLES, &MSQuality)))
			PresentParams.MultiSampleType = D3DMULTISAMPLE_4_SAMPLES;
		else if (SUCCEEDED(ProxyInterface->CheckDeviceMultiSampleType(Adapter, DeviceType, PresentParams.BackBufferFormat, PresentParams.Windowed, D3DMULTISAMPLE_2_SAMPLES, &MSQuality)))
	...
	*ppReturnedDeviceInterface = new Direct3DDevice8(this, DeviceInterface, BehaviorFlags, &PresentParams);
