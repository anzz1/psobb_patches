...
DOF::DOF(IDirect3DDevice9 *device, int width, int height)
    ...
    defines.push_back(pixelSizeMacro);
		...
		/**
		 * IMPORTANT! Here we load and compile the DOF effect from a *RESOURCE*
		...
		V(D3DXCreateEffectFromResourceA(device, g_hModule, "DOF.fx", &defines.front(), &includeResource, flags, nullptr, &effect, nullptr));
		//V(D3DXCreateEffectFromFileW(device, L"DOF.fx", &defines.front(), nullptr, flags, nullptr, &effect, nullptr));
...
void DOF::go(IDirect3DTexture9 *frame, IDirect3DTexture9 *depth, IDirect3DSurface9 *dst) {
    ...
    // And here we go!
    dofPass(frame, depth, dst);
}

void DOF::dofPass(IDirect3DTexture9* frame, IDirect3DTexture9 *depth, IDirect3DSurface9 *dst) {
    D3DPERF_BeginEvent(D3DCOLOR_XRGB(0, 0, 0), L"DOF: 1st pass");
    HRESULT hr;
    ...
    V(effect->SetFloat(dofAmtHandle, 6.0f));
...
void DOF::quad(int width, int height) {
